...
Quantum Purse is a quantum-safe wallet for [CKB blockchain](https://www.nervos.org/). It started out in the form of a static web application and now has fully been built into a native PC app across multiple platforms. Any deployment mentioned after this only means serving Quantum Purse code for your browser remotely!
...
##### <u>Recommendation</u>
For CKB:
- 's' variant is on-chain friendly as it's fast and light weight. The tradeoff here is that key generation and signing on Quantum Purse takes longer to execute.
...
- One new account starts syncing at block 0 will render other accounts sync to wait until this account catches up. Provide starting blocks properly for a smooth experience.
...
For the complete command list, refer to [Geting started](#getting-started) section. Currently, the following are the recommended ways to use this software, ranked from most to least preferred:
1. If you're a normal user, download QuantumPurse in the release page version ^0.2.1 to download, verify SHA sum against the published one, then use it.
2. If possible, build a native pc app via `npm run build:app:<platform>` then run the binary in `build/` folder.
...
## Contribution
Any PR to `develop` branch is welcomed. Have an idea or have found an issue? Feel free to leave a message in the [Quantum Purse telegram group](https://t.me/quantumpurse) or open a github issue to let me know.
