...
Quantum Purse is a quantum-safe wallet for CKB blockchain. It started out in the form of a static web application and now has fully been built into a native PC app. Any deployment mentioned after this only means serving Quantum Purse code for your browser remotely!
...
4. For maximum security (seems a little paranoid though xd), use an air-gapped device (one with no internet connection) to run Quantum Purse. To sign a transaction:
   ...
   - ALTHOUGH THIS IS NOT SUPPORT YET, let me see if CKB community really wants this feature or not. 