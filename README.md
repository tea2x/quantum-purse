...
Quantum Purse is a lightweight quantum-safe wallet for [CKB blockchain](https://www.nervos.org/).
...
- In case you notice that sync percentage grows very slowly for some reason e.g creating wallet without internet, please set `starting block` to skip blocks that have none transaction.
...
1. If you're a normal user, download QuantumPurse in the release page version ^0.2.1 to download.
2. If possible, build from source code `npm run build:app:<platform>` then run the binary in `build/` folder.
